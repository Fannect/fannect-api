...
app.get "/v1/teams/:team_id/huddles", auth.rookieStatus, (req, res, next) ->
   ...
   async.parallel pull, (err, results) ->
      ...
      if created_by == "any"
         query = Huddle.or([
            { "team_id", team_id }
            { "tags.include_key": team.conference_key },
            { "tags.include_key": team.league_key },
      ...
      .limit(limit)
      .skip(skip)
      .select("owner_id owner_user_id owner_name owner_verified topic reply_count replies team_name rating rating_count last_comment_time")
      .exec (err, huddles) ->
         return next(new MongoError(err)) if err
         res.json huddles
...
app.post "/v1/teams/:team_id/huddles", auth.rookieStatus, (req, res, next) ->
   ...
   async.parallel
      team: (done) ->
         Team.findById(team_id, "team_name league_key league_name conference_key conference_name", done)
      teams: (done) ->
