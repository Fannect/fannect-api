...
app.get "/v1/me/teams", auth.rookieStatus, (req, res, next) ->
   ...
   .select("team_id team_name team_key points")
...
app.get "/v1/me/teams/:team_profile_id", auth.rookieStatus, (req, res, next) ->
   TeamProfile.findById req.params.team_profile_id, 
      { user_id: 1, name: 1, profile_image_url: 1, team_id: 1, team_image_url: 1, team_name: 1, points: 1, shouts: { $slice: [-1, 1]}, is_college: 1, friends_count: 1, rank: 1 }
   , (err, profile) ->
      return next(new MongoError(err)) if err
